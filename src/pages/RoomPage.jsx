...
export default function RoomPage() {
  ...
  const { startLocal, stopLocal, hasAudio, setHasAudio, hasVideo, setHasVideo, localStream } =
    useLocalMedia();
  const { peers, attachLocalStream, toggleTrack, leaveAll, setSelfId } = useWebRTC(socket, roomId);
  ...
  React.useEffect(() => {
    ...
    const onConnect = () => { void doJoin(); };
    socket.on("connect", onConnect);

    if (socket.connected) { void doJoin(); }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <span>My userId:</span>
        <code style={{ padding: "2px 6px", background: "#f5f5f5", borderRadius: 4 }}>
          {peerIdRef.current}
        </code>
        <button onClick={copyId} title="คัดลอก userId">Copy</button>
      ...
      >
        <VideoTile stream={localStream} muted label={`You (${shortId(peerIdRef.current)})`} />
